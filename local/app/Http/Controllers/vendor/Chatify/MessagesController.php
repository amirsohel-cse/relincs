...
class MessagesController extends Controller
{
    ...
    public function idFetchData(Request $request)
    {
        ...
        if ($request['type'] == 'user') {
            $fetch = User::select('username','image_profile','id','active_status')->find($request['id']);
            ddd($fetch);
            if($fetch){
                $userAvatar = "https://relincs.com/storage/storage/profile_image/".$fetch->image_profile;
            }
        }
    ...
    {
        $path = storage_path() . 'storage/storage/attachments/' . config('chatify.attachments.folder') . '/' . $fileName;
        if (file_exists($path)) {
    ...
    public function getFavorites(Request $request)
    {
        ...
        foreach ($favorites->get() as $favorite) {
            ...
            $favoritesList .= view('Chatify::layouts.favorite', compact('user'));
        }
        // send the response
        return Response::json([
    ...
    public function sharedPhotos(Request $request)
    {
        ...
        for ($i = 0; $i < count($shared); $i++) {
            $sharedPhotos .= view('Chatify::layouts.listItem', [
                'get' => 'sharedPhoto',
                'image' => asset('storage/storage/attachments/' . $shared[$i]),
            ])->render();
        }
        // send the response
        return Response::json([
