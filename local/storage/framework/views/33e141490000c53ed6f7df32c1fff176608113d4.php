...
<a class="flex w-3/3 h-80 justify-center items-center shadow-md cursor-pointer rounded-md object-cover"
    href="<?php echo e(route('show.wall.feed',$data->uid)); ?>" x-data="{ hover : false , details : false }" @click="details = true"
    @mouseover="hover = true"
    style="margin:0px 10px 0px 10px;box-shadow: 10px 10px 23px 0px rgba(51,46,46,0.75);
-webkit-box-shadow: 10px 10px 23px 0px rgba(51,46,46,0.75);
-moz-box-shadow: 10px 10px 23px 0px rgba(51,46,46,0.75);">
    <div class="w-full h-full relative rounded-md">
            <?php if(str_contains($data->path,'.mp4')): ?>
                <img src="<?php echo e(env('AWS_BUCKET_URL').'video/'. $data->uid.'/'.$data->thumbnail_image); ?>" alt="<?php echo e($data->title); ?>"
                    class="rounded-sm w-full h-full object-cover">
                <?php else: ?>
                <img src="<?php echo e(env('AWS_BUCKET_URL'). $data->path); ?>" alt="<?php echo e($data->title); ?>"
                    class="rounded-sm w-full h-full object-cover">
            <?php endif; ?>
...
<?php /**PATH /var/www/relincs.com/htdocs/local/resources/views/components/card-wall.blade.php ENDPATH**/ ?>