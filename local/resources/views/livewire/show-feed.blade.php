        ...
        <div class="lg:w-10/12 w-full">
            <div class="flex w-full justify-between items-center px-2 my-1 mb-2 z-50">
                <div class="absolute left-0 transform ">
                    @if ($prev !== null)
                    <a href="{{ route('show.wall.feed',$prev->uid) }}" @mouseover.away="prev = false"
                        ...
                        <span x-transition x-show="prev" class="mx-1 pl-3">{{$prev->title}}</span>
                    </a>
                    @endif
                </div>
                <div class="absolute right-0 transform ">
                    @if ($next !== null)
                    <a href="{{ route('show.wall.feed',$next->uid) }}" @mouseover.away="next = false"
                        ...
                        <span x-transition x-show="next" class="mx-1 pl-3">{{$next->title}}</span>
          ...
          {{-- {{dd($video)}} --}}
            <div class="relative w-full h-full flex justify-center items-center" wire:ignore>
                ...
                <div class="w-full h-full rounded-sm">
                    <img src="{{ env('AWS_BUCKET_URL').$video->path }}" class="bg-center object-cover rounded-md"
                ...
                <video id="watch" class="video-js vjs-fluid vjs-styles=default vjs-big-play-centered z-10"
                    ...
                    poster="{{ env('AWS_BUCKET_URL').'video/'.$video->uid.'/'.$video->thumbnail_image }}"
                    >
                    <source src="{{ env('AWS_BUCKET_URL').'video/'.$video->uid.'/'.$video->processed_file }}" >
                    <p class="vjs-no-js">
            ...
            <div class="w-full">
                <!-- Comment -->
                @livewire('all-comments', ['video' => $video])
                ...
                <div class="w-full absolute bg-black bg-opacity-75 inset-0 shadow-lg transform transition duration-100 z-30"
                    ...
                    <div
                        ...
                        @livewire('components.tools-wall', ['video' => $video], key($video->id))
                    ...
                    <div class="flex flex-col w-full">
                        <div class="flex justify-start flex-col space-y-4 items-start pb-2 px-3">
                            <div class="relative mt-1 w-full flex flex-col">
                                <h1 class="text-md font-bold text-black">{{$video->title}}</h1>
                        ...
                        <div class="p-2 pt-1">
                            <p class="font-normal text-xs text-gray-500">{{ $video->created_at->diffForHumans() }}</p>
                        </div>
                    </div>
                </div>
            </div>
