...
<a class="flex w-3/3 h-80 justify-center items-center shadow-md cursor-pointer rounded-md object-cover"
    href="{{ route('show.wall.feed',$data->uid) }}" x-data="{ hover : false , details : false }" @click="details = true"
...
-moz-box-shadow: 10px 10px 23px 0px rgba(51,46,46,0.75);">
    <div class="w-full h-full relative rounded-md">
            @if(str_contains($data->path,'.mp4'))
                <img src="{{ env('AWS_BUCKET_URL').'video/'. $data->uid.'/'.$data->thumbnail_image }}" alt="{{ $data->title }}"
                    class="rounded-sm w-full h-full object-cover">
                @else
                <img src="{{ env('AWS_BUCKET_URL'). $data->path }}" alt="{{ $data->title }}"
                    class="rounded-sm w-full h-full object-cover">
            @endif
            <div class="absolute inset-0 hover:bg-gray-900 hover:bg-opacity-50 transition-all duration-200">
                <div class="flex text-white flex-col h-full w-full justify-center self-center items-center space-y-2"
                    ...
                    <h1 class="text-lg mt-2">{{$data->title}}</h1>
                </div>
            </div>
    </div>
